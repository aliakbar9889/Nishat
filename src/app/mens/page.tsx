...
import { mensProductsQuery } from '@/sanity/lib/query';
...
const ShopPage = () => {
  ...
  const hardcodedLinks = [
    '/product22', '/product23', '/product16', '/product13',
    '/product42', '/product37', '/product29', '/product28',
  ];

  // fetchData memoized with dependencies
  const fetchData = useCallback(async () => {
    try {
      let query = mensProductsQuery;
      const params: QueryParams = {};

      if (searchTerm) {
        query = `*[_type == "product" && category == "men" && lower(title) match lower($searchTerm)]`;
  ...
  // useEffect me debounce function define kiya hai
  useEffect(() => {
  ...
  }, [searchTerm, sortOption, fetchData]);

  const handleSearchIconClick = () => {
    fetchData(); // Immediate fetch without debounce
  };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex justify-start w-32 sm:w-40 relative">
            <input
              ...
              placeholder="Search mens products..."
          ...
          <div className="text-center flex-1">
            ...
            <p className="text-gray-500 mt-2 text-sm md:text-base">Discover the latest mens products at unbeatable prices.</p>
          </div>
